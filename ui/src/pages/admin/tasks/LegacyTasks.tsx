...
import { LoaderCircle, ClockIcon, CheckIcon, FileTextIcon, FileXIcon, Upload } from "lucide-react";
    ...
    try {
      ...
      if (isCsv) {
        ...
        const headers = Array.isArray(headerRow?.values)
          ? headerRow.values.slice(1).map((value) => normalizeText(String(value ?? "")))
          : [];
        parsedRows = [];
        worksheet?.eachRow((row, rowNumber) => {
          ...
          headers.forEach((header, index) => {
            normalized[header] = String(row.getCell(index + 1).value ?? "").trim();
          });
