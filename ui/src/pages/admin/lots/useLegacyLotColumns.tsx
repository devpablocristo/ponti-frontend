...
export function useLegacyLotColumns({
  getFilterOptionsForColumn,
  onSuccessEdit,
}: UseLegacyLotColumnsArgs) {
  return useMemo(() => {
    const baseColumns: Column<LotsData>[] = [
      ...
      },
      {
        ...
        render: (crop) => (
          <span className="text-gray-900 dark:text-slate-100">{String(crop ?? "")}</span>
        ),
      },
      {
        key: "current_crop",
      ...
      },
      {
        ...
        render: (value) => (
          <span className="font-bold text-gray-900 dark:text-slate-100">
            {formatNumericValue(value)}{" "}
            <span className="text-xs font-normal text-gray-900 dark:text-slate-100">
              Tn/Has
            </span>
          </span>
        ),
