              ...
              {columns.map((column, index) => {
                ...
                return (
                  <th
                    ...
                    className={`${headerAlignmentClass(column)} ${paddingClass(column.headerPadding)} ${column.headerWrap ? "whitespace-normal break-words" : ""
                      ...
                      {enableFilters && column.filterable !== false ? (
                        ...
                        >
                          <button
                            ...
                            className={`relative ml-1 focus:outline-none ${activeFilter === String(column.key) || isFilterActive
                                ? "text-primary-500"
                                  ...
                                  <>
                                    ...
                                    <div className="max-h-48 overflow-auto pr-1 text-slate-600">
                                      {(() => {
                                        ...
                                        const selectedOptions = Array.isArray(current) ? current : [];

                                        const allVisibleSelected =
                                          visibleOptions.length > 0 &&
                                          visibleOptions.every((option) => selectedOptions.includes(option));

                                        const someVisibleSelected =
                                          visibleOptions.some((option) => selectedOptions.includes(option)) &&
                                          !allVisibleSelected;

                                        const toggleAllVisibleOptions = (checked: boolean) => {
                                          const next = checked
                                            ? [...new Set([...selectedOptions, ...visibleOptions])]
                                            : selectedOptions.filter((option: string) => !visibleOptions.includes(option));
                                                  ...
                                                  <input
                                                    ...
                                                    onChange={(event) => {
                                                      const next = event.target.checked
                                                        ...
                                                        : selectedOptions.filter((value: string) => value !== option);
                          ...
                          <div className="flex justify-center space-x-2">
                            ...
                            {onEdit ? (
                              <button
                                ...
                                className={`mr-3 font-medium ${(canEdit?.(item) ?? true) === false
