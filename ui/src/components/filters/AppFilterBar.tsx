...
function inputSizeClass(size: "sm" | "md") {
...
function selectedOptionFor(filter: FilterItem) {
  if (filter.value === null || filter.value === undefined || filter.value === "") {
    return undefined;
  }
  return filter.options?.find((option) => String(option.id) === String(filter.value));
}

function displayValueFor(filter: FilterItem) {
  if (filter.type === "select") {
    const selectedOption = selectedOptionFor(filter);
    return selectedOption ? optionLabel(selectedOption) : "";
  }
...
function ActionButtonView({ action, size }: { action: ActionButton; size: "sm" | "md" }) {
  return (
    <Button
      variant={action.variant ?? (action.isPrimary ? "primary" : "outlinePonti")}
      size={size}
    ...
    <div className="w-full">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`input-base block w-full pl-9 pr-8 ${inputSizeClass(size)} ${
...
function FilterSuggestions({
  ...
  return (
    <ul className="absolute top-full z-50 mt-1 max-h-[240px] w-full overflow-y-auto rounded-xl border border-slate-200 bg-white py-1 shadow-lg dark:border-slate-700 dark:bg-slate-800">
      {allowAll ? (
        <li
          className="cursor-pointer px-3.5 py-2.5 text-sm font-medium text-slate-600 transition-colors duration-150 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700"
      ...
      {!filter.loading && !filter.error && options.length === 0 ? (
...
export function AppFilterBar({
  ...
  const handleSuggestionClick = useCallback(
    (filter: FilterItem, option: FilterOption) => {
      ...
      filter.onChange(optionLabel(option));
  ...
  const createHandleKeyDown = useCallback(
    (filter: FilterItem) => (event: KeyboardEvent<HTMLInputElement>) => {
  ...
  return (
    <div className={`relative z-40 w-full ${className}`.trim()}>
                ...
                <div className="relative">
                  <SearchInput
                    ...
                    onChange={(event) => {
                      ...
                      setSearchByFilter((prev) => ({ ...prev, [filter.name]: nextValue }));
                      setSuggestionsVisible((prev) => ({ ...prev, [filter.name]: true }));
