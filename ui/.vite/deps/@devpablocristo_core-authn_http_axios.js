...
// node_modules/@devpablocristo/core-authn/src/browser/events.ts
var AUTH_FORCE_LOGOUT_EVENT = "auth:force-logout";
...
function dispatchAuthForceLogout(target) {
...
// node_modules/@devpablocristo/core-authn/src/http/axios.ts
function defaultInvalidTokenMatcher(error) {
  var _a, _b;
  ...
  const data = (_b = axiosError.response) == null ? void 0 : _b.data;
  const haystack = typeof data === "string" ? data : data && typeof data === "object" ? JSON.stringify(data) : "";
  const message = haystack.toLowerCase();
  return message.includes("invalid token") || message.includes("token inval") || message.includes("token invál") || message.includes("jwt") || message.includes("signature") || message.includes("expired");
}
var AuthenticatedAxiosClient = class {
  constructor(options) {
    __publicField(this, "client");
    __publicField(this, "tokenStorage");
    __publicField(this, "refreshRequest");
    __publicField(this, "invalidTokenMatcher");
    __publicField(this, "isRefreshing", false);
    __publicField(this, "failedQueue", []);
    __publicField(this, "attachToken", (config) => {
      const useRefreshToken = this.isRefreshRequest(config) && this.refreshRequest.useRefreshToken;
      const token = useRefreshToken ? this.tokenStorage.getRefreshToken() : this.tokenStorage.getAccessToken();
      if (token) {
    ...
    __publicField(this, "handleError", async (error) => {
      var _a, _b, _c;
      const originalRequest = error.config;
      if (this.invalidTokenMatcher(error) && ((_a = error.response) == null ? void 0 : _a.status) === 403) {
        this.forceLogout();
        return Promise.reject(error);
      }
      if (originalRequest && ((_b = error.response) == null ? void 0 : _b.status) === 401 && originalRequest._retry) {
        this.forceLogout();
        return Promise.reject(error);
      }
      if (!originalRequest || ((_c = error.response) == null ? void 0 : _c.status) !== 401 || originalRequest._retry || this.isRefreshRequest(originalRequest)) {
        return Promise.reject(error);
      }
  ...
  async post(endpoint, data, config) {
    const headers = data instanceof FormData ? { ...(config == null ? void 0 : config.headers) ?? {}, "Content-Type": "multipart/form-data" } : config == null ? void 0 : config.headers;
  ...
  isRefreshRequest(config) {
    return !!(config == null ? void 0 : config.url) && config.url === this.refreshRequest.path;
  }
  async refreshTokens() {
    const method = this.refreshRequest.method.toUpperCase();
    const response = method === "POST" ? await this.client.post(this.refreshRequest.path, this.refreshRequest.body ?? {}) : await this.client.get(this.refreshRequest.path);
...
function createAuthenticatedAxiosClient(options) {
...
//# sourceMappingURL=@devpablocristo_core-authn_http_axios.js.map
